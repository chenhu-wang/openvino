...
GroupNormalizationDecomposition::GroupNormalizationDecomposition() {
    ...
    ov::matcher_pass_callback callback = [=](ov::pass::pattern::Matcher& m) {
        ...
        if (data.get_element_type() != element::f32) {
            reshaped_node = std::make_shared<ov::snippets::op::ConvertSaturation>(reshaped_node, element::f32);
        }
        ...
        auto reduce_mean_variance = std::make_shared<ov::op::v1::Multiply>(mean_sum_variance, group_size_inv_node_aux);
        ...
        std::vector<Dimension> group1_dims(group_rank);
