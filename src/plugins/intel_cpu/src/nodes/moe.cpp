...
struct MOE::Executor : MOE::ExecutorBase {
    ...
    Executor(const dnnl::engine& engine,
        ...
        // specialized 2nd-token kernel with more fused ops. second token?
        linear_up
        ...
        auto do_reorder = [&](Weight* pw, onednn_matmul& mm) {
            ...
            if (pw->zp) {
                ...
                if (pw->raw_data_dtype == memory::data_type::u4) {
                    ...
                    auto* psrc0 = pw->zp;
                    auto* psrc1 = static_cast<int8_t*>(new_src.get_data_handle());  // convert to int8
    ...
    void execute(const dnnl::stream& strm, MOE* pnode) override {
        ...
        scratch_expert_weight.resize<float>({n_tokens, pnode->m_config.topk});
        {
            parallel_for(n_tokens, [&](int i) {
                ...
                // softmax
                float denorm = 0;
                float vmax = max_heap[0].second;  // is this max
        ...
        if (n_tokens > 1) {
            ...
            for (int i = 0; i < expert_num; i++) {
                ...
                max_num_toks = std::max(max_num_toks, num_tokens);      // how many assigned tokens for one expert(with largest tokens)
            }
            auto max_batch_size = normalize_batch_size(max_num_toks);   // aligned
            if (scratch_batch_size < max_batch_size) {                  // scratch_batch_size initialized? if (max_batch_size > 0), allocate
                scratch_batch_size = max_batch_size;
                // for gathered tokens
                scratch_src = memory({{scratch_batch_size, hidden_size}, m_act_dtype, memory::format_tag::ab}, engine);
                // up and gate have same weight?
                scratch_up =
            ...
            for (int expert_id = 0; expert_id < expert_num; expert_id++) {
                ...
                // for each expert, num_tokens is different, so need create kernels in runtime(can be cached).
                auto& nks = get_nkernels(engine, expert_batch_size);

                // gather from all tokens
                std::atomic<int> slot(0);
                // loop all tokens' feature for each expert.
                parallel_for(n_tokens, [&](int i) {
                ...
                parallel_for(num_tokens, [&](int seq) {
                    ...
                    case memory::data_type::bf16: {
                        ...
                        auto* dst = reinterpret_cast<ov::bfloat16*>(final_hidden_states) + i * n_states;  // scatter
...
void MOE::extractConsts() {
    ...
    m_weights.resize(consts.size());  // resize to expert num
